...
// ----------------------------------------------------------------------------------------
    ...
    void png_loader::read_image( const char* filename )
    {
        ...
        {
            fclose( fp );
            throw image_load_error(std::string("png_loader ld_->png_ptr_ == NULL: parse error in file ") + filename);
        }
        ...
        if ( ld_->info_ptr_ == NULL )
        {
            ...
            throw image_load_error(std::string("png_loader ld_->info_ptr_ == NULL: parse error in file ") + filename);
        }
